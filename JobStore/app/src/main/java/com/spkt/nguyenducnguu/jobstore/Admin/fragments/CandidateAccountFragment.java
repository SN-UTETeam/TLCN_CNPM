...
import com.spkt.nguyenducnguu.jobstore.Admin.adapters.ManageAccountCandidateAdapter;
...
public class CandidateAccountFragment extends Fragment {
    ...
    RecyclerView rv_CandidateAccount;
    ManageAccountCandidateAdapter mAdapter;
    TextView txt_Filter, txt_NumberResult;
    EditText txt_Query;
    List<Candidate> lstData = new ArrayList<>();

    public CandidateAccountFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_candidate_account, container, false);
    ...
    private void addView(View rootView) {
        rv_CandidateAccount = (RecyclerView) rootView.findViewById(R.id.rv_CandidateAccount);
                ...
                Database.getData(Node.CANDIDATES, new OnGetDataListener() {
                    ...
                    public void onSuccess(DataSnapshot dataSnapshot) {
                        ...
                        for (DataSnapshot mdata : dataSnapshot.getChildren()) {
                            Candidate can = mdata.getValue(Candidate.class);
                            can.setKey(mdata.getKey());
                            if (mSetting.checkStatus(can.getStatus()) && mSetting.checkDate(can.getCreateAt())) {
                                lstData.add(can);
                                mAdapter.notifyDataSetChanged();
                                mAdapter.FilterData(mSetting);
                                txt_NumberResult.setText(lstData.size() + "/" + lstData.size());
    ...
    private void addOnScrolled() {
        rv_CandidateAccount.addOnScrollListener(new RecyclerView.OnScrollListener() {
    ...
    private void setmRecyclerView() {
        mAdapter = new ManageAccountCandidateAdapter(lstData);

        LinearLayoutManager layoutManager = new LinearLayoutManager(getContext());
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);

        rv_CandidateAccount.setLayoutManager(layoutManager);
        // ta sẽ set setHasFixedSize bằng True để tăng performance
        rv_CandidateAccount.setHasFixedSize(true);
        rv_CandidateAccount.setAdapter(mAdapter);
    }
